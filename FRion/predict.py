...
def predict():
    ...
    parser.add_argument(
        ...
        help=(
            "Generate channel frequencies (in Hz): \n"
            "    minfreq, maxfreq, channel_width"
        ),
    )
    parser.add_argument(
        "-t", dest="telescope_name", type=str, default=None, help="Telescope name"
    )
    parser.add_argument(
        "-T",
        dest="telescope_coords",
        nargs=3,
        type=float,
        default=None,
    ...
    if args.fits is not None:
        start_time, end_time, freq_arr, telescope, ra, dec = get_parms_from_FITS(
            args.fits
        )
    ...
    if start_time is None:
        missing_parms.append("Start time")
    if end_time is None:
        missing_parms.append("End time")
    if freq_arr is None:
        missing_parms.append("Frequency array")
    if telescope is None:
        missing_parms.append("Telescope")
    if ra is None:
        missing_parms.append("Pointing center")
    ...
    if args.savefig is not None:
        generate_plots(
            times, RMs, theta, freq_arr, position=[ra, dec], savename=args.savefig
        )
...
def calculate_modulation(
    ...
    theta = numeric_integration(times.mjd * 86400.0, RMs, frequencies.to(u.Hz).value)
...
def get_RM(
    ...
    if type(ra) == float or type(ra) == int:
        ...
        raise Exception(
            """RA input object type not recognized.
    ...
    if type(dec) == float or type(dec) == int:
        ...
        raise Exception(
            """Dec input object type not recognized.
                        Only astropy.coordinates Angle, astropy.units Quantity, or float (in deg) allowed."""
        )
    ...
    if type(timestep) == float or type(timestep) == int:
        ...
        raise Exception(
            """Timestep input object type not recognized.
    ...
    times = Time(predictions["times"] / 86400.0, format="mjd")
...
def numeric_integration(times, RMs, freq_array):
    ...
    z = np.exp(2.0j * np.outer(l2_arr, RMs))
    ...
    theta = (real + 1.0j * imag) / (times[-1] - times[0])
    return theta
...
def get_telescope_coordinates(telescope):
    ...
    """
    if (
        type(telescope) == EarthLocation
    ):  # Pass EarthLocations through without processing
        return telescope
...
def get_parms_from_FITS(filename):
    ...
    if "FITSTYPE" in header.keys() and header["FITSTYPE"] == "PSRFITS":
        ...
        if "TELESCOP" in header.keys():
            try:
                ...
                telescope = EarthLocation.from_geocentric(
                    header["ANT_X"], header["ANT_Y"], header["ANT_Z"], unit=u.m
                )
    ...
    # FITS images/cubes:
        ...
        if "DATE-OBS" in header.keys() and "DURATION" in header.keys():
            end_time = Time(header["DATE-OBS"]) + TimeDelta(
                header["DURATION"], format="sec"
            )
        ...
        if freq_axis != "0":
            chan0 = header["CRVAL" + freq_axis] - header["CDELT" + freq_axis] * (
                header["CRPIX" + freq_axis] - 1
            )
            chan_final = (
                chan0 + (header["NAXIS" + freq_axis] - 1) * header["CDELT" + freq_axis]
            )
